...
def tabulate(args: argparse.Namespace) -> None:
    ...
    logging.info(f'generating tabulated {args.entries=} at '
                 f'p=np.linspace(*{args.pressure}) [Pa], '
                 f'T=np.linspace(*{args.temperature}) [K]')

    qs = Quantities(p=np.linspace(*args.pressure),
                    T=np.linspace(*args.temperature))
    header = (f'low ({qs.p[0]} {qs.T[0]}); '
              f'high ({qs.p[-1]} {qs.T[-1]}); '
              'values ')
    with concurrent.futures.ProcessPoolExecutor() as e:
        ...
        for future in concurrent.futures.as_completed(future_to_outfile):
            ...
            try:
                ...
                logging.warning(
                    f'writing to {outfile.name=} raised an {exception=}')
